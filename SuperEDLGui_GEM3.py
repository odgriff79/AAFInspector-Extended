...
def extract_metadata(mob_node):
    """Extracts SourceClipStartFrames and URLString from a SourceMob."""
    ...
    # Extract Start from Timecode segment
    ...
    if slots_node and len(slots_node) > 3:
        for slot in slots_node[3]:
            ...
            segment_node = next((c for c in slot[3] if isinstance(c, list) and c[0] == "Segment"), None)
            if not segment_node or len(segment_node) < 4: continue
            timecode_node = next((c for c in segment_node[3] if isinstance(c, list) and c[0] == "Timecode"), None)
            if timecode_node and len(timecode_node) > 3:
                start_node = next((c for c in timecode_node[3] if isinstance(c, list) and c[0] == "Start"), None)
                if start_node and len(start_node) > 2:
                    try:
                        metadata["SourceClipStartFrames"] = int(start_node[2])
                        break
...
def get_clip_name(mob_map, mobid):
    ...
    if url: return os.path.basename(urllib.parse.unquote(urllib
...
class App:
    ...
    def process(self):
        ...
        sequence_mob, start_tc = find_main_sequence_mob_and_start_tc(self.json_data)
        if not sequence_mob:
        ...
        events = recursive_search(sequence_mob, timeline_offset=start_tc)
        self.log_msg(f"   - Events found: {len(events)}")
        if not events:
            self.log_msg("❌ No clip events were found.")
            return
