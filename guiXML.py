...
def sanitize_for_xml(text):
    ...
    if not isinstance(text, str):
        return ""
    # Encode to ASCII, ignoring any character that can't be represented,
    # then decode back to a clean string. This guarantees removal of unwanted symbols.
...
def generate_fcpxml(summary_info, events_data, output_path):
    ...
    seq_start_frames = get_frames_from_tc_string(events_data[0]['Timeline Start TC'])
    last_event = max(events_data, key=lambda x: get_frames_from_tc_string(x.get('Timeline Start TC', '0')) + int(x.get('Event Length', 0)))
        ...
        if animated_params and clip_element is not None:
            ...
            if pos_x_param and pos_y_param:
                ...
                for kf_x in pos_x_param[1]:
                    ...
                    kf_time = frames_to_fcpxml_time(kf_x['frame'] - clip_timeline_start_frames, timeline_rate)
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("CSV to FCPXML Converter (FINAL)")
