...
- Keyframes are emitted in ASSET TIME **including source offset**:
      KF = asset_src_abs + src_off + local_frames
- Asset durations in <resources> are expanded to cover the used window:
      duration >= src_off + evt_len
  so all KFs are inside [asset.start, asset.start + duration]
- Guaranteed-closed XML blocks (adjust-crop/transform/blend), no dangling tags.

Keeps:
- Strict DF/NDF converter for diagnostics/EDL
- Position in % of height (default) or pixels
- Emit rotation only when non-constant non-zero
- Head & inter-event gaps with start="3600/1s"
"""
...
# ----------------- strict timecode + frame fraction helpers -----------------

def tc_to_frames(tc: str, fps: float) -> int:
    """NDF parse: HH:MM:SS:FF (or ;) for CSV fields only."""
...
def frames_to_den(frames: int, fps: float) -> Tuple[int,int]:
    """Represent frames as n/den seconds. Denominator is nominal int FPS."""
    den = int(round(fps))
    return int(frames), den

def frames_to_tc_precise(frames: int, fps: float, drop: bool=False) -> str:
    """Precise DF/NDF for EDL diagnostics."""
    f = int(round(frames))
    ...
    frames_per_10m = nominal * 60 * 10
    frames_per_min = nominal * 60

    d = f // frames_per_10m
    m = f % frames_per_10m
    dropped = drop_frames * (9 * d + max(0, ((m - drop_frames) // frames_per_min)))
    f_adj = f + dropped

    ff = f_adj % nominal
    sec_total = f_adj // nominal
    s = sec_total % 60
    m = (sec_total // 60) % 60
    h = sec_total // 3600
    return f"{h:02d};{m:02d};{s:02d};{ff:02d}"
...
def frames_to_tc(frames: int, fps: float, drop_frame: bool=False, df: Optional[bool]=None) -> str:
    ...
    return frames_to_tc_precise(frames, fps, drop_frame)
...
def read_csv_summary_and_events(path: Path):
    ...
    if sep is None: raise ValueError("CSV missing blank line between summary and table.")
...
def parse_summary(summary_lines: List[str]):
    ...
    for ln in summary_lines:
        if ln.startswith("Timeline Name,"):
            ...
            if len(rhs)>1: tcfmt = rhs[1].strip("()").upper()
...
def classify_param_name(pname: str):
    if not pname: return None, None
    if pname in AFX_MAP: return AFX_MAP[pname]
    low = pname.lower().strip()
...
def avid_pos_to_pixels(px_raw: Any, py_raw: Any, width: int, height: int):
    ax = to_float_safe(px_raw); ay = to_float_safe(py_raw)
    if ax is None and ay is None: return None, None
    rx = ax*(width/1000.0) if ax is not None else None       # Avid 1000 = full width
    ry = -(ay*(height/1000.0)) if ay is not None else None   # Y up normalization
    return rx, ry

def pixels_to_percentH(px: Optional[float], py: Optional[float], height: int):
...
def parse_keyframes_block(kf_text: str):
    ...
    for raw in txt.splitlines():
        ...
        m = KF_PARAM_RE.match(line)
        if m:
            cur = m.group(1).strip()
            if in_anim: animated.setdefault(cur, [])
            continue
        if in_anim and cur:
            m2 = KF_LINE_RE.search(line)
            if m2:
                animated[cur].append((int(m2.group(1)), m2.group(2).strip()))
    for k in animated: animated[k].sort(key=lambda t: t[0])
    return animated

def group_raw_kf_by_time(animated: Dict[str, List[Tuple[int,str]]]):
    ...
    grouped: "OrderedDict[int, Dict[str, Any]]" = OrderedDict(
        (t, {'posx':None,'posy':None,'scalex':None,'scaley':None,'rot':None,'op':None,
             'cropleft':None,'cropright':None,'croptop':None,'cropbottom':None}) for t in times
    )
    for pname, pts in animated.items():
        ...
        for t,val in pts:
            ...
            elif cls=="rot": g['rot']=val
            elif cls=="opacity": g['op']=val
            elif cls=="crop":
...
class EventModel:
    def __init__(self):
        ...
        self.evt_abs = 0; self.evt_len = 0; self.src_off = 0
        self.asset_src_abs = 0; self.asset_src_len = 0; self.asset_tc_start_frames = 0
        self.kf_raw_grouped: "OrderedDict[int, Dict[str, Any]]" = OrderedDict()
        # resolved tracks (local frames → string values)
...
def build_model_from_csv(csv_path: Path, timeline_width: int=1920, timeline_height: int=1080,
    ...
    # Filter out filler/placeholders
    rows: List[Dict[str,str]] = []
    for r in events_csv:
        nm  = str(r.get("Event Name","") or "")
        mob = str(r.get("SourceMobID","") or "")
        clip= str(r.get("Clip Name","") or "")
        if ("Filler" in nm) or ("FX_ON_FILLER" in mob) or ("placeholder" in clip):
            continue
        rows.append(r)

    # Sort by timeline start
    rows = sorted(rows, key=lambda r: tc_to_frames(r.get("Timeline Start TC","00:00:00:00"), fps))
    ...
    for r in rows:
        ...
        for absf, comp in M.kf_raw_grouped.items():
            ...
            if sx is None and sy is None and comp['scalex'] is not None:
                s = convert_for_fcpxml("scale", comp['scalex'])
                sx, sy = s, s
    ...
    header = {
        "name": name, "fps": fps, "tc_format": tc_format, "drop_frame": drop_frame,
        "seq_start_tc": seq_start_tc, "seq_start_frames": seq_start_frames,
        "seq_length_frames": seq_length_frames, "head_gap_frames": head_gap_frames,
        "timeline_width": int(timeline_width), "timeline_height": int(timeline_height),
        "pos_units": pos_units,
    }
    return header, models

# ----------------- KF time (asset-time + source offset) -----------------

def effective_asset_duration_frames(M: EventModel) -> int:
    """
    Ensure the asset resource covers the used window:
        used_window = src_off + max(1, evt_len)
        effective = max(asset_src_len (if known), used_window)
    """
    used_window = (M.src_off or 0) + max(1, M.evt_len or 0)
    known = M.asset_src_len or 0
    return max(known, used_window)

def kf_time_asset_with_offset(M: EventModel, local_frames: int, fps: float,
                              effective_len: Optional[int]=None) -> str:
    """
    KF in asset time, including source offset:
        abs = asset_src_abs + src_off + local
    Clamp to:
        [asset_src_abs + src_off, asset_src_abs + src_off + min(effective_len, evt_len) - 1]
    """
    local = int(local_frames)
    if local < 0: local = 0
    if M.evt_len > 0 and local > M.evt_len - 1:
        local = M.evt_len - 1

    min_abs = M.asset_src_abs + M.src_off
    # cap by both effective asset length and event length
    if effective_len is None:
        effective_len = effective_asset_duration_frames(M)
    max_abs = min_abs + max(0, min(M.evt_len or 0, effective_len) - 1)

    abs_frames = min_abs + local
    if abs_frames < min_abs: abs_frames = min_abs
    if abs_frames > max_abs: abs_frames = max_abs
...
def write_fcpxml_from_model(header: Dict[str,Any], models: List[EventModel], out_path: Path,
    ...
    """
    time_mode kept for CLI compatibility; we always emit asset-time + src offset for KFs.
    """
    ...
    # Precompute effective asset durations for clamping & <asset> duration
    eff_len_by_idx: Dict[int,int] = {i: effective_asset_duration_frames(M) for i,M in enumerate(models)}

    def values_change(track: List[Tuple[int,str]]) -> bool:
        if len(track) < 2: return False
        first = track[0][1].strip()
        return any(v.strip() != first for _,v in track[1:])

    def format_kf_time(M: EventModel, local_frames: int, eff_len: int) -> str:
        return kf_time_asset_with_offset(M, local_frames, fps, effective_len=eff_len)
    ...
    # Assets (we emit one per model index to guarantee coverage; deduping is optional)
    rid = 2
    ...
    for i,M in enumerate(models):
        ...
        # duration must cover src_off + evt_len even if original length was short
        dur_frames = eff_len_by_idx[i]
        n2,d2 = frames_to_den(dur_frames, fps)
    ...
    L += [
        '    </resources>',
        '    <library>',
        f'        <event name="{sanitize(name)}">',
        f'            <project name="{sanitize(name)}">'
    ]
    ...
    # Head gap with start="3600/1s"
    if head_gap_frames > 0:
        ng,dg = frames_to_den(head_gap_frames, fps)
        L.append(f'                        <gap name="Gap" offset="{nseq}/{dseq}s" start="3600/1s" duration="{ng}/{dg}s"/>')

    def write_gap_at(offset_frames: int, duration_frames: int):
        if duration_frames <= 0:
            return
        on, od = frames_to_den(offset_frames, fps)
        dn, dd = frames_to_den(duration_frames, fps)
        L.append(f'                        <gap name="Gap" offset="{on}/{od}s" start="3600/1s" duration="{dn}/{dd}s"/>')

    prev_end = seq_start_frames + head_gap_frames

    for i,M in enumerate(models):
        # inter-event gap
        if M.evt_abs > prev_end:
            write_gap_at(prev_end, M.evt_abs - prev_end)

        asset_id = asset_id_by_index.get(i)
        off_n,off_d = frames_to_den(M.evt_abs, fps)
        dur_n,dur_d = frames_to_den(M.evt_len, fps)
        ref_attr = f' ref="{asset_id}"' if asset_id else ''
        L.append(f'                        <video name="{M.clip_name}" offset="{off_n}/{off_d}s" start="0/1s" duration="{dur_n}/{dur_d}s" enabled="1"{ref_attr}>')

        # ---- CROP (always properly closed) ----
        if export_crop and (M.tr_crop_left or M.tr_crop_right or M.tr_crop_top or M.tr_crop_bottom):
            ...
            def write_crop(side: str, track: List[Tuple[int,str]]):
                ...
                for local, val in track:
                    ts = format_kf_time(M, local, eff_len_by_idx[i])
                    L.append(f'                                            <keyframe time="{ts}" value="{val}"/>')
                L.append('                                        </keyframeAnimation>')
                L.append('                                    </param>')
        ...
        # ---- TRANSFORM (always opened & closed) ----
        base_scale_attr = M.tr_scale[0][1] if M.tr_scale else "1 1"
        ...
        if M.tr_scale:
            L.append(f'                                <param value="{M.tr_scale[0][1]}" name="scale">')
            L.append(f'                                    <keyframeAnimation>')
        ...
        if M.tr_position:
            pos_changes = values_change(M.tr_position)
            L.append(f'                                <param value="{M.tr_position[0][1]}" name="position">')
            L.append(f'                                    <keyframeAnimation>')
        ...
        # Rotation only if meaningful
        def rotation_emit_track(track: List[Tuple[int,str]]) -> bool:
            ...
            if all(is_zero(v) for _,v in track): return False
            first = track[0][1].strip()
            if all(v.strip()==first for _,v in track) and is_zero(first):
                return False
            return True

        if rotation_emit_track(M.tr_rotation):
            L.append(f'                                <param value="{M.tr_rotation[0][1]}" name="rotation">')
            L.append(f'                                    <keyframeAnimation>')
...
def write_marker_edl_from_model(header: Dict[str,Any], models: List[EventModel],
    ...
    for idx, M in enumerate(models, start=1):
        ...
        for t, comp in M.kf_raw_grouped.items():
            ...
            if comp['rot'] is not None:
                avid_parts.append(f"rotation={comp['rot']}")
...
def main() -> None:
    ...
    ap.add_argument("-x","--xml",   required=True, type=Path)
    ap.add_argument("-e","--edl",   required=True, type=Path)
    ...
    ap.add_argument("--time-mode", choices=["resolve_valid","abs","local"], default="resolve_valid",
                    help="Kept for compatibility; keyframes are always asset-time + source offset.")
    ...
    write_marker_edl_from_model(header, models, args.edl,
                                max_kf_per_event=args.max_kf)
