...
- Matches Resolve-style absolute keyframe times (e.g. 3600/1s, 90366/25s) when time_mode=resolve_valid
- Inserts head/inter-event gaps with start="3600/1s"
- Omits rotation if constant 0; omits adjust-blend unless present/animated
- Adds curve="linear" to position kfs only when values change
- Position units default to percent-of-height; switchable to pixels

Usage:
  python csv_to_fcpxml_correct_TCs_FX_EDL_TIME_MODE_CROP_UNITS_v4.py \
    -i mem1.csv \
    -x mem1_REL13.fcpxml \
    -e mem1_REL15_markers.edl \
    --time-mode resolve_valid \
    --pos-units percentH \
...
def frames_to_tc_df(frames: int, fps: float) -> str:
    # 29.97 & 59.94 support; otherwise fallback to NDF-like
...
def avid_pos_to_pixels(px_raw: Any, py_raw: Any, width: int, height: int):
    ...
    rx = ax*(width/1000.0) if ax is not None else None       # Avid units: 1000 == full width
    ry = -(ay*(height/1000.0)) if ay is not None else None   # Avid Y: up positive → Resolve Y up positive so negate pixel
    return rx, ry
...
class EventModel:
    def __init__(self):
        ...
        # flip/flop flags (not used here; could be extended)
        self.scale_sign_x = 1; self.scale_sign_y = 1
...
def build_model_from_csv(csv_path: Path, timeline_width: int=1920, timeline_height: int=1080,
    ...
    # Filter out filler and placeholders
    ...
    # Head gap (from sequence start to first event)
    ...
    for r in rows:
        ...
        # roll forward to build tracks (local frames)
        ...
        for absf, comp in M.kf_raw_grouped.items():
            ...
            # position (Avid 1000 units → pixels → percent-of-height (default))
...
def write_fcpxml_from_model(header: Dict[str,Any], models: List[EventModel], out_path: Path,
    ...
    def format_kf_time(M: EventModel, local_frames: int) -> str:
        # Resolve-valid: absolute *sequence* time expressed as “Source Clip TC start + local”
        if time_mode == "resolve_valid":
            base_secs = M.asset_tc_start_frames / fps
            secs = base_secs + (local_frames / fps)
            # use /1s on exact seconds to match Resolve exports
            ...
            t = max(M.evt_abs, min(M.evt_abs + max(0,M.evt_len-1), M.evt_abs + local_frames))
    ...
    # Assets (optional; will be used if paths are present)
    rid = 2
    ...
    for i,M in enumerate(models):
        # add inter-event gap if needed
        if M.evt_abs > prev_end:
            write_gap_at(prev_end, M.evt_abs - prev_end)

        asset_id = asset_id_by_index.get(i)
        # Even if we have no asset block, we still write <video> with ref if available
        ...
        # TRANSFORM (base attributes mirror first keyframe)
        ...
        # Rotation: omit if constant zero / no change from zero
