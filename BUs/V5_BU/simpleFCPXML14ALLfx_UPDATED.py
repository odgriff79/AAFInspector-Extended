...
A Tkinter GUI that:
  ...
  - Generates a structurally correct FCPXML with fully working keyframe animations,
    applying the correct conversion schema for Avid AFX and DVE effect types.
"""
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        for i, event in enumerate(events_to_process):
            ...
            if has_transform:
                ...
                first_pos_x = (pos_x_kfs[0]['value'] / 1920.0) if pos_x_kfs else 0
                first_pos_y = (pos_y_kfs[0]['value'] / 1080.0) if pos_y_kfs else 0
                ...
                if pos_x_kfs or pos_y_kfs:
                    ...
                    for frame in all_pos_frames:
                        x = next((k['value'] for k in pos_x_kfs if k['time'] == frame), first_pos_x * 1920.0)
                        y = next((k['value'] for k in pos_y_kfs if k['time'] == frame), first_pos_y * 1080.0)
                        y_inv = y * -1 if is_dve else y
                        f.write(f'                                        <keyframe time="{frame}/{int(fps)}s" value="{x / 1920.0} {y_inv / 1080.0}" curve="linear"/>\n')
