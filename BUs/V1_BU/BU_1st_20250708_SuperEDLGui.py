...
def extract_metadata(mob_node):
    """
    Finds all 'Start' values in a Mob and intelligently selects the correct
    one corresponding to the Avid Bin Start TC (usually the largest value).
    """
    metadata = {"SourceClipStartFrames": 0} # Default to 0
    ...
    if all_starts:
        # The correct start time is usually the largest value found
...
class App:
    ...
    def process(self):
        ...
        for idx, e in enumerate(events, 1):
            ...
            enriched.append({
                "Event": idx,
                "ClipName": get_clip_name_from_mob_id(mob_map, e["MobID"]),
        ...
        self.log_msg("\n--- Super EDL Report ---")
        header = f"{'#':<4} {'Timeline In':<15} {'Source Clip Start':<20} {'Offset':<15} {'Event Source In':<20} {'Clip Name'}"
        self.log_msg(header); self.log_msg("-" * (len(header) + 5))
        for r in enriched:
            line = f"{r['Event']:<4} {r['TimelineInTC']:<15} {r['SourceClipStartTC']:<20} {r['SourceOffsetTC']:<15} {r['SourceClipEventInTC']:<20} {r['ClipName']}"
            self.log_msg(line)
