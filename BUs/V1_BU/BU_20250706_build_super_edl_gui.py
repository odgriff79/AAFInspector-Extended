    ...
    if name == "Sequence":
        for c in children:
            if isinstance(c, list) and c[0] == "Components":
                for comp in c[3]:
                    recursive_search(comp, timeline_offset, edit_rate, results, depth+1, counters, log_file, dedupe_set)
    if name == "SourceClip":
        mobid = ""
        source_start = 0
        length = 0
        ...
        if dedupe_key not in dedupe_set:
            ...
            if log_file:
                log_file.write(f"{indent}🎯 Unique SourceClip: MobID={mobid}, Start={source_start}, TimelineStart={timeline_offset}, Length={length}, EditRate={edit_rate}\n")
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("AAF Super EDL Generator v2")
