...
# -*- coding: utf-8 -*-
"""
csv_to_fcpxml_correct_TCs_FX_EDL_TIME_MODE_CROP_UNITS_v3.py

CSV → FCPXML (REL13) + Marker EDL (REL15)

Locked to Resolve-valid behavior + valid-example parity:
- Keyframe times: source_tc_start_seconds + (local_frames / fps)  (default --time-mode=resolve_valid)
- Position units: percent-of-height (default --pos-units=percentH) e.g. "3.7 -13.875"
- <video ... start="0/1s"> and <param value="..." name="...">
- curve="linear" applied *only* to POSITION keyframes (and only if >1 KF)
- No curve attribute on scale/rotation/crop/opacity keyframes

CLI flags retained: --time-mode (resolve_valid|abs|local), --pos-units (percentH|pixels)
"""
...
# ---------------- Flip/Flop ----------------
def parse_flip_flags(effect_name: str):
...
def build_model_from_csv(csv_path: Path, timeline_width: int=1920, timeline_height: int=1080,
    ...
    for r in rows:
        ...
        # flip/flop → sign on scale (clip + filler)
        h_clip, v_clip = parse_flip_flags(M.effect_name)
        h_sign, v_sign = (-1 if h_clip else 1), (-1 if v_clip else 1)
        for _fs,_fe,_fh,_fv in []:  # reserved for filler segments if you expand
            pass
...
def write_fcpxml_from_model(header: Dict[str,Any], models: List[EventModel], out_path: Path,
    ...
    def values_change(track: List[Tuple[int,str]], eps: float=1e-6) -> bool:
    ...
    def write_kf_param(M: EventModel, name: str, track: List[Tuple[int,str]]):
        ...
        add_linear_curve = (name == "position") and values_change(track)
    ...
    for i,M in enumerate(models):
        ...
        L.append(f'                        <video name="{M.clip_name}" offset="{off_n}/{off_d}s" start="0/1s" duration="{dur_n}/{dur_d}s" enabled="1" ref="{asset_id}">')

        # CROP (no curve attributes)
        ...
        # TRANSFORM (anchor first, then base attrs)
        ...
        # Emit in same order as valid example: scale → position → rotation
