...
A Tkinter GUI that:
  ...
  - Generates a structurally correct FCPXML with fully working keyframe animations for
    static, single-keyframe, and animated effects.
  - Uses the correct original source clip duration.
"""
...
def parse_keyframe_details(kf_string, clip_start_frames):
    ...
    for line in kf_string.strip().split('\n'):
        # Match parameter name for both animated and static lines
        ...
        if static_kf_match and not kf_match and current_param:
            ...
            try:
                value = float(value_str.split('/')[0]) / float(value_str.split('/')[1]) if '/' in value_str else float(value_str)
                # Static effects are treated as a single keyframe at the start of the clip
...
def create_fcpxml():
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        for i, event in enumerate(events_to_process):
            ...
            # --- UPDATED: Use original source clip length ---
            source_duration_frames = int(event.get('Orig Source Clip length', 0))
            duration_str = "0/1s" if is_still else f"{source_duration_frames}/{int(fps)}s"
        ...
        for i, event in enumerate(events_to_process):
            ...
            base_attrs = f'start="{clip_start_in_source_frames}/{int(fps)}s" ref="{asset_ref_id}" offset="{clip_offset}/{int(fps)}s" duration="{clip_duration_frames}/{int(fps)}s" name="{clip_name}" enabled="1"'
            full_attrs = base_attrs if is_still else f'{base_attrs} tcFormat="{tc_format}" format="r0"'
            
            f.write(f'                        <{tag} {full_attrs}>\n')
            ...
            if not has_transform and not crop_kfs and not opacity_kfs:
                ...
                if has_transform:
                    ...
                    pos_x_kfs = sorted(transform_kfs.get('position_x', []), key=lambda k: k['time'])
                    pos_y_kfs = sorted(transform_kfs.get('position_y', []), key=lambda k: k['time'])
                    scale_kfs = sorted(transform_kfs.get('scale', []), key=lambda k: k['time'])
                    rot_kfs = sorted(transform_kfs.get('rotation', []), key=lambda k: k['time'])
                    ...
                    if pos_x_kfs or pos_y_kfs:
                        all_pos_frames = sorted(list(set(k['time'] for k in (pos_x_kfs or []) + (pos_y_kfs or []))))
                        f.write(f'                                <param value="{first_pos_x} {pos_y_init}" name="Position">\n                                    <keyframeAnimation>\n')
                        for frame in all_pos_frames:
                            x = next((k['value'] for k in (pos_x_kfs or []) if k['time'] == frame), first_pos_x)
                            y = next((k['value'] for k in (pos_y_kfs or []) if k['time'] == frame), first_pos_y)
                            y_inv = y * -1 if is_dve else y
                            f.write(f'                                        <keyframe time="{frame}/{int(fps)}s" value="{x} {y_inv}" curve="linear"/>\n')
                        f.write('                                    </keyframeAnimation>\n                                </param>\n')

                    if scale_kfs:
                        f.write(f'                                <param value="{first_scale} {scale_y_init}" name="Scale">\n                                    <keyframeAnimation>\n')
                        for kf in scale_kfs:
                            val = kf['value'] if is_pan_zoom else kf['value'] / 100.0
                            y_val = val * -1 if is_dve else val
                            f.write(f'                                        <keyframe time="{kf["time"]}/{int(fps)}s" value="{val} {y_val}" curve="linear"/>\n')
                        f.write('                                    </keyframeAnimation>\n                                </param>\n')
                    
                    if rot_kfs:
                        f.write(f'                                <param value="{first_rot}" name="Rotation">\n                                    <keyframeAnimation>\n')
                        for kf in rot_kfs:
                            f.write(f'                                        <keyframe time="{kf["time"]}/{int(fps)}s" value="{kf["value"] * -1}" curve="linear"/>\n')
                        f.write('                                    </keyframeAnimation>\n                                </param>\n')
                    f.write('                            </adjust-transform>\n')
                else: # Has other effects but no transform
                    f.write('                            <adjust-transform position="0 0" scale="1 1" anchor="0 0"/>\n')

                if crop_kfs:
                    f.write('                            <filter-video ref="r0" name="Crop">\n')
                    for param_name in ['left', 'right', 'top', 'bottom']:
                        ...
                        f.write(f'                                <param value="{initial_val}" name="{param_name}">\n')
                        if kfs_list:
                            f.write('                                    <keyframeAnimation>\n')
                            for kf in kfs_list:
                                f.write(f'                                        <keyframe time="{kf["time"]}/{int(fps)}s" value="{kf["value"]}" curve="linear"/>\n')
                            f.write('                                    </keyframeAnimation>\n')
                        f.write('                                </param>\n')
                ...
                if opacity_kfs:
                    ...
                    f.write('                            <filter-video ref="r0" name="Opacity">\n')
                    f.write(f'                                <param value="{initial_opacity}" name="opacity">\n')
                    if op_kfs_list:
                        f.write('                                    <keyframeAnimation>\n')
