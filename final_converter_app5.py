...
def build_xmeml(header_text, events):
    ...
    ET.SubElement(s, 'name').text = f"{clean_text(seq_name)} (Converted)"
    ...
    for ev in events:
        ...
        try:
            ...
            continue
        cid += 1
        # clipitem id uses Source File Name and zero-based index
        item_index = cid - 1
        ...
        # file element id uses same name with index and '2'
        ...
        reel_name = name
        if dl and len(dl)>len(tp): reel_name = dl
        elif tp: reel_name = tp
        ...
        # Default filters block omitted for brevity; same as previous
        # ...
