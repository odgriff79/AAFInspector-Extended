    ...
    def process(self):
        ...
        for idx, e in enumerate(events, start=1):
            ...
            if is_override:
                ...
                # --- MODIFIED: Get correct source duration from the Master Mob ---
                orig_length = 0
                master_mob = None
                master_mob_id = None
                
                # 'initial' is the clip on the timeline. Find the Master Mob it refers to via its SourceID.
                if initial and len(initial) > 3:
                    slots_node = next((c for c in initial[3] if c[0] == "Slots"), None)
                    if slots_node and len(slots_node) > 3 and slots_node[3]:
                        # Find the first video slot to get the SourceID from its SourceClip
                        for slot in slots_node[3]:
                            segment = next((c for c in slot[3] if c[0] == "Segment"), None)
                            if not segment: continue

                            source_clip_node = None
                            # The segment's data is usually a Sequence containing a SourceClip
                            if len(segment) > 3 and isinstance(segment[3], list) and segment[3] and segment[3][0][0] == 'Sequence':
                                components = next((c for c in segment[3][0][3] if c[0] == 'Components'), None)
                                if components and len(components) > 3 and isinstance(components[3], list) and components[3] and components[3][0][0] == 'SourceClip':
                                    source_clip_node = components[3][0]
                            
                            if source_clip_node:
                                master_mob_id = next((c[2] for c in source_clip_node[3] if c[0] == "SourceID"), None)
                                if master_mob_id:
                                    master_mob = mob_map.get(master_mob_id)
                                    break # Found Master Mob, no need to check other slots

                if master_mob:
                    # Now, find the 'Length' within the Master Mob's EssenceDescriptor
                    essence_desc = next((c for c in master_mob[3] if isinstance(c, list) and c[0] == 'EssenceDescription'), None)
                    if essence_desc and len(essence_desc) > 3 and isinstance(essence_desc[3], list) and essence_desc[3]:
                        descriptor_node = essence_desc[3][0] # This gets the CDCIDescriptor
                        if isinstance(descriptor_node, list) and len(descriptor_node) > 3:
                            length_node = next((c for c in descriptor_node[3] if isinstance(c, list) and c[0] == 'Length'), None)
                            if length_node and len(length_node) > 2:
                                try:
                                    orig_length = int(length_node[2])
                                except (ValueError, TypeError):
                                    pass # orig_length remains 0
