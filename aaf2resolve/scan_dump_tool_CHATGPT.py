#!/usr/bin/env python3
# scan_dump_tool_CHATGPT.py — DIRECT AAF → CSV mirroring the legacy JSON walker
# This build includes:
# - UMID normalization ({}-_.: whitespace stripped)
# - Shallow SourceID hopping (slot.segment + immediate children only) with zero-UMID skipping
# - “Final or last-good mob” fallback (return mob with locators/descriptor when chain breaks)
# - StartTime via properties() (StartTime/start_time/start) instead of node.start only
# - Geometry columns (StoredWidth, StoredHeight, ImageAspectRatio)
# - Keyframe details plus clip-relative timing ("time + clip-in")
# - Locator harvesting from fields AND locator.properties(); file:// → UNC/Windows normalization
# - Robust MobID capture via attr + properties()
...
def normalize_file_url_to_path(url: str) -> str:
    """
    Convert AAF NetworkLocator URLString → filesystem path with UNC normalization.
    Examples:
      file://RadiantNexis00/share/folder/file.mxf → //RadiantNexis00/share/folder/file.mxf
      file:///C:/path/file.mxf                        → C:/path/file.mxf
      file://10.10.1.195/share/folder/file.mxf       → //10.10.1.195/share/folder/file.mxf
    """
    s = str(url or "")
    ...
    # UNC-like: '/Server/Share' should become '//Server/Share'
    if path.startswith("/") and not path.startswith("//"):
        path = "/" + path
    # Windows local: '/C:/path' -> 'C:/path'
...
def decode_locator_url_to_path(url):
    try:
        ...
        if s.lower().startswith("file://"):
            return normalize_file_url_to_path(s)
        return s
    except:
...
def stringify_mobid(val) -> str:
    """Try to stringify pyaaf2 MobID-like value into a URN-ish or readable string."""
    try:
        if val is None:
            return ""
...
def render_fx_details(animated, staticp, clip_in_frames=None, fpsF=Fraction(25,1)):
    lines = []
    if animated:
        lines.append('--- Animated Parameters ---')
        for pname, pts in animated.items():
            lines.append(f"  - {pname} ({len(pts)} keyframes)")
            for (t, v) in pts:
                t_str = fraction_to_str(t)
                v_str = fraction_to_str(v)
                if clip_in_frames is not None:
                    # best-effort "+ clip-in" presentation; we don't assume units → show expression
                    lines.append(f"    Time: {t_str} + clip-in {clip_in_frames} -> Value: {v_str}")
                else:
                    lines.append(f"    Time: {t_str} -> Value: {v_str}")
    if staticp:
        if lines: lines.append("")
        lines.append('--- Static Parameters ---')
        for pname, v in staticp.items():
            lines.append(f"  - {pname}: {fraction_to_str(v)}")
...
def get_start_time_frames(component):
    """Prefer StartTime/start_time/start via properties(); fallback to .start; returns int frames if possible."""
    v = _find_prop(component, ["StartTime", "start_time", "start"])
    if v is None:
        v = getattr(component, "start", None)
    try:
        return int(v)
    except:
        # if rational-like, try to convert
        try:
            return int(Fraction(getattr(v, "numerator", 0), getattr(v, "denominator", 1)))
        except:
            return 0

def _get_source_id_from_sourceclip(src_clip):
    val = _find_prop(src_clip, ["SourceID", "source_id"])
    if val is None:
        val = getattr(src_clip, "source_id", None)
    if val is None:
        return None
    return stringify_mobid(val)

def find_source_id(component):
    return _get_source_id_from_sourceclip(component)
...
def _has_locators(mob):
    try:
        desc = getattr(mob, "descriptor", None)
        if not desc:
            return False
        locs = getattr(desc, "locators", None) or []
        return len(list(locs)) > 0
    except:
        return False

def _extract_locators_any(mob, dbg: DebugLog):
    paths = []
    try:
        desc = getattr(mob, "descriptor", None)
        if not desc:
            return paths
        # Primary fields on locator objects
        for loc in getattr(desc, "locators", []) or []:
            for fld in ("path", "url", "URLString", "UrlString", "PathName"):
                if hasattr(loc, fld):
                    v = getattr(loc, fld, None)
                    if v:
                        s = decode_locator_url_to_path(v)
                        if s and s not in paths:
                            paths.append(s)
                            if dbg: dbg.write(f"        [locator:{fld}] {s}")
            # Also scan locator.properties() for URL-ish fields
            try:
                for p in loc.properties():
                    nm = safe_name(getattr(p, "name", ""))
                    if nm in ("URLString", "PathName", "path", "url"):
                        s = decode_locator_url_to_path(getattr(p, "value", None))
                        if s and s not in paths:
                            paths.append(s)
                            if dbg: dbg.write(f"        [locator.prop:{nm}] {s}")
            except:
                pass
    except:
        pass
...
def extract_mob_locators_tape_disk(mob, dbg: DebugLog):
    locs = _extract_locators_any(mob, dbg)
    tape = ""
    disk = ""
    try:
...
# ==============================================================================
#    ROBUST SOURCE RESOLVER — shallow hop + "final or last-good mob" fallback
# ==============================================================================
...
def resolve_genuine_source_from_sourceclip(src_clip, mob_map, dbg=None):
    """
    JSON-parity Source resolver with faithful fallback:
    - Begin at SourceClip.SourceID mob
    - For each hop, examine preferred slot.segment:
        • If segment is SourceClip → use its SourceID
        • If segment is Sequence/OperationGroup → ONLY inspect immediate children in order, and
          choose the first SourceClip whose SourceID is NON-ZERO
    - Keep last mob that has locators/descriptor
    - If chain breaks or next UMID is zero/missing/loop → return last-good mob (or current)
    - If FileSourceMob reached → return it
    """
    ...
    visited = set()
    last_with_loc = cur if _has_locators(cur) else None
    ...
    while cur is not None:
        ...
        if _has_locators(cur):
            last_with_loc = cur

        if aaf_class_name(cur) == "FileSourceMob":
            if dbg: dbg.write("      Reached FileSourceMob; returning it (terminal).")
            return cur
        ...
        if not next_id:
            if dbg: dbg.write("      No valid shallow SourceClip hop; returning last mob with locators (or current).")
            return last_with_loc or cur

        nxt = mob_map.get(normalize_umid(next_id)) or mob_map.get(next_id)
        if not nxt:
            if dbg: dbg.write(f"      Next mob {next_id} missing; returning last mob with locators (or current).")
            return last_with_loc or cur
    ...
    with open(out_txt_path, "a", encoding="utf-8") as f:
        ...
        def visit(node, rec):
            ...
            if cname == "OperationGroup":
                ...
                animated, staticp = _fx_points(node)
                for k, v in staticp.items():
                    w(f"      static: {k} = {fraction_to_str(v)}")
                for k, lst in animated.items():
    ...
    for label, it in coll:
        ...
        for mob in it:
            try:
                ...
                # Also capture MobID via properties() if present
                try:
...
def super_edl_from_sequence(seq_mob, mob_map, out_csv_path, dbg: DebugLog):
    ...
    def record_fx(rec_in, op_group):
        ...
        animated, staticp = _fx_points(op_group)
    ...
    def visit(node, rec, track_id_counter):
        ...
        if cname == "OperationGroup":
            ...
            if is_fx_on_filler:
                ...
                # compute keyframe details now (no clip-in offset here)
                kf = render_fx_details(
                    fx_by_rec_in.get(rec, [{}])[-1].get("animated", {}),
                    fx_by_rec_in.get(rec, [{}])[-1].get("staticp", {}),
                    clip_in_frames=None,
                    fpsF=fpsF
                )
                events.append({
                    ...
                    "StoredWidth": "", "StoredHeight": "", "ImageAspectRatio": ""
                })
            else:
                for attr in ("components", "input_segments"):
                    for ch in list(getattr(node, attr, []) or []):
        ...
        if cname == "SourceClip":
            ...
            # Robust resolver (shallow hop + "final or last-good" fallback; try ALL immediate SCs)
            fsm = resolve_genuine_source_from_sourceclip(node, mob_map, dbg)
            ...
            if fsm:
                ...
                descriptor = getattr(fsm, 'descriptor', None)
                try:
                    if descriptor and getattr(descriptor, "length", None) is not None:
                        orig_length = int(getattr(descriptor, "length"))
                except:
                    orig_length = 0
                # Geometry (best-effort; some descriptors won't have these)
                try:
                    dw = getattr(descriptor, "stored_width", None)
                    dh = getattr(descriptor, "stored_height", None)
                    ar = getattr(descriptor, "image_aspect_ratio", None)
                    stored_w = safe_name(dw) if dw is not None else ""
                    stored_h = safe_name(dh) if dh is not None else ""
                    aspect = fraction_to_str(ar) if ar is not None else ""
                except:
                    pass
            else:
                dbg.write(f"    WARNING: Could not resolve genuine source (even with fallback). MobID: {source_mob_id}")

            off = get_start_time_frames(node)  # <-- properties() aware
            ...
            # Merge FX at this rec with clip-in aware keyframe strings
            fx_list = fx_by_rec_in.get(rec, [])
            if fx_list:
                # combine all fx for this rec
                names = []
                merged_lines = []
                for fx in fx_list:
                    names.append(fx["name"])
                    merged_lines.append(
                        render_fx_details(fx["animated"], fx["staticp"], clip_in_frames=off, fpsF=fpsF)
                    )
                effect_name_out = "; ".join(names)
                keyframe_details_out = "\n\n".join(merged_lines)
            else:
                effect_name_out = "N/A"
                keyframe_details_out = "No effect data found."
            ...
            events.append({
                ...
                "Effect Name": effect_name_out,
                "Keyframe Details": keyframe_details_out,
                "Orig Source Clip length": orig_length,
                "StoredWidth": stored_w, "StoredHeight": stored_h, "ImageAspectRatio": aspect
            })
            return
    ...
    with open(out_csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        w.writerow(["Timeline Edit Rate", f"{float(seq_rate)} (NDF)" if not drop else f"{float(seq_rate)} (DF)"])
