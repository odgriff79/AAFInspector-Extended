...
def extract_aaf(aaf_path: str, csv_path: str):
    ...
    # Write CSV
    if not events:
        print("⚠️  Warning: No SourceClip events were found in the AAF file.")
        print(f"An empty CSV file will be created at: {csv_path}")
        # Create an empty file to indicate the script ran but found nothing
        with open(csv_path, "w") as out:
         pass
    else:
     # This block now only runs if the events list is not empty
        print(f"✅ Found {len(events)} events. Writing to CSV...")
        with open(csv_path, "w", newline="", encoding="utf-8") as out:
         # Define fieldnames from the first event's keys, which is now safe
