...
class App:
    ...
    def process(self):
        ...
        timeline_rate = next((c[2] for c in sequence_mob[3] if c[0] == "EditRate"), 25.0)
        timeline_len = next((c[2] for c in sequence_mob[3] if c[0] == "Length"), 0)
        
        summary_info = {
            "Timeline Name": next((c[2] for c in sequence_mob[3] if c[0] == "Name"), "N/A"),
            "Timeline Edit Rate": float(timeline_rate),
            "Timeline Start TC": frames_to_tc(start_tc, timeline_rate),
            "Timeline Length": f"{frames_to_tc(timeline_len, timeline_rate)} ({timeline_len} frames)"
        }
        ...
        for idx, e in enumerate(events, 1):
            ...
            enriched.append({
                "Event Number": idx,
                "Clip Name": clip_name,
                "Source File Name": clip_name, # Often same as Clip Name
