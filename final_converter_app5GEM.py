...
# This mapping uses the exact column names from your CSV header
COLUMN_MAPPING = {
    "clip_name": "Clip Name",
...
def create_xmeml_final(df, logger):
    ...
    # ✅ FIX: Precisely filter out only true filler/slugs
    original_row_count = len(df)
    filler_conditions = (df[C['clip_name']].str.lower() == 'filler') | (df[C['clip_name']].str.lower() == 'slug')
    df_no_filler = df[~filler_conditions].copy()
    
    if len(df_no_filler) < original_row_count:
        logger(f"INFO: Skipped {original_row_count - len(df_no_filler)} filler/slug rows.")

    if df_no_filler.empty:
    ...
    for col_name in numeric_cols:
        df_no_filler[col_name] = pd.to_numeric(df_no_filler[col_name], errors='coerce').fillna(0)

    sequence_start_offset = df_no_filler[C['start_time_frames']].min()
    sequence_rate_val = df_no_filler.iloc[0][C['edit_rate']]
    
    ET.SubElement(sequence, 'name').text = "Definitive_Sequence_v25"
    last_clip = df_no_filler.loc[df_no_filler[C['start_time_frames']].idxmax()]
    sequence_duration = (last_clip[C['start_time_frames']] - sequence_start_offset) + last_clip[C['event_length']]
    ...
    ET.SubElement(timecode_seq, 'string').text = clean_text(df_no_filler.iloc[0][C['timeline_start_tc']])
    ET.SubElement(timecode_seq, 'frame').text = str(int(sequence_start_offset))
    ...
    for i, row in df_no_filler.iterrows():
        start_frame = row[C['start_time_frames']] - sequence_start_offset
        ...
        ET.SubElement(clipitem, 'start').text = str(int(start_frame))
        ET.SubElement(clipitem, 'end').text = str(int(start_frame + duration))
...
class XMLExporterApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Definitive Converter v25")
    ...
    def load_csv(self):
        ...
        if path:
            try:
                # ✅ FIX: Intelligent loading logic
                # First, try loading as a normal CSV (like 'test 2.csv')
                temp_df = pd.read_csv(path)
                required_cols = set(COLUMN_MAPPING.values())
                
                # If the required columns aren't found, try loading with skiprows=8
                if not required_cols.issubset(temp_df.columns):
                    self.log("Normal header not found, attempting to load with skiprows=8...")
                    temp_df = pd.read_csv(path, skiprows=8)
    ...
    def run_generation(self):
        ...
        try:
            # Pass the logger function to the backend for more detailed feedback
