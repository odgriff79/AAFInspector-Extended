...
A Tkinter GUI that:
  ...
  - Generates a structurally correct FCPXML with fully working keyframe animations,
    including correct value conversions for Avid AFX and DVE effect types.
"""
...
#
# This script uses the following logic to convert Avid effect parameters
# from a CSV report into values compatible with DaVinci Resolve's FCPXML format.
#
# 1. POSITION (X, Y)
#    - Avid provides pixel offsets from a center point.
#    - Resolve uses a normalized coordinate system where the frame's width/height is 1.0.
#    - Conversion (assuming a 1920x1080 frame):
#      - Resolve_X = Avid_X / 1920
#      - Resolve_Y = Avid_Y / 1080
#    - For DVE-type effects, the Y-axis is inverted: Resolve_Y = (Avid_Y / 1080) * -1
#
# 2. SCALE (Zoom)
#    - AFX Scale is a percentage (e.g., 100.0 = 1x scale).
#    - Pan & Zoom 'Zoom Factor' is a direct multiplier (e.g., 1.0 = 1x scale).
#    - DVE Scale is a percentage with an inverted Y-axis.
#    - Conversion:
#      - AFX: Resolve_Zoom = Avid_Scale / 100.0
#      - Pan & Zoom: Resolve_Zoom = Avid_Zoom_Factor
#      - DVE: Resolve_Zoom_X = Avid_Scale / 100.0, Resolve_Zoom_Y = (Avid_Scale / 100.0) * -1
#
# 3. CROP (Left, Right, Top, Bottom)
#    - Both Avid and Resolve use direct pixel values for cropping.
#    - Conversion: Direct 1-to-1 mapping.
#
# 4. ROTATION
#    - Avid and Resolve use degrees, but Avid's direction is inverted.
#    - Conversion: Resolve_Rotation = Avid_Rotation * -1
#
# 5. OPACITY
#    - Both systems use a 0-100 percentage scale.
#    - Conversion: Direct 1-to-1 mapping.
#
# --- End of Schema ---
...
def create_fcpxml():
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        for i, event in enumerate(events_to_process):
            ...
            duration_str = "0/1s" if is_still else f"{int(event.get('Orig Source Clip length', fps * 3600))}/{int(fps)}s"
