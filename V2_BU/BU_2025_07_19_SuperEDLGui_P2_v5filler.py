...
def frames_to_tc(frame_count, fps=25.0, is_drop_frame=False):
    """
    Converts a frame count to a timecode string, handling Drop-Frame format.
    """
...
def has_nested_source_clip(node):
    """Helper function to recursively check for a nested SourceClip."""
...
def recursive_search(node, timeline_offset=0, edit_rate=25, results=None, dedupe_set=None):
    ...
    if name == "Sequence":
        ...
        # If the OperationGroup contains a clip, just recurse to find it.
        if has_nested_source_clip(node):
            for c in children:
                recursive_search(c, timeline_offset, edit_rate, results, dedupe_set)
        # Otherwise, it's an effect on filler. Log it as a Pan & Zoom event.
        else:
            length = next((int(c[2]) for c in children if c[0] == "Length"), 0)
            file_path = "N/A"
            file_param = next((c for c in children if c[0] == "Parameters"), None)
            if file_param and len(file_param) > 3:
                for p in file_param[3]:
                    if isinstance(p, list) and "URLString" in str(p):
                        url_node = next((x for x in p[3] if x[0] == "URLString"), None)
                        if url_node and len(url_node) > 2:
                            file_path = urllib.parse.unquote(urllib.parse.urlparse(url_node[2]).path)
                            break
            results.append({
    ...
    def process(self):
        ...
        for idx, e in enumerate(events, 1):
            if e.get("MobID") == "PanZoomFiller":
                ...
                enriched.append({
                    ...
                    "Source File Name": source_file_name, "Source File Path": os.path.dirname(file_path) if file_path != "N/A" and file_path else "N/A",
        ...
        for r in enriched:
            ...
            if r.get("SourceMobID") == "Effect":
                 ...
                 self.log_msg(f"  Clip Name: {r['Clip Name']}")
