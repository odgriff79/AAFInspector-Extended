        ...
        for i, event in enumerate(events_to_process):
            ...
            if not keyframes:
                ...
                for param, kfs in keyframes.items():
                    ...
                    elif 'SCALE_X' in p_upper or 'SCALE_Y' in p_upper: transform_kfs.setdefault('scale', []).extend(kfs)
                    elif 'ZOOM FACTOR' in p_upper: transform_kfs.setdefault('zoom', []).extend(kfs)
                ...
                scale_kfs = transform_kfs.get('scale') or transform_kfs.get('zoom')
                rot_kfs = transform_kfs.get('rotation')
                is_pan_zoom = bool(transform_kfs.get('zoom'))
                ...
                if crop_kfs:
                    ...
                    for param_name in ['left', 'right', 'top', 'bottom']:
                        kfs_list = crop_kfs.get(param_name)
                ...
                if opacity_kfs:
                    op_kfs_list = opacity_kfs.get('opacity')
