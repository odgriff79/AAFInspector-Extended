...
class TreeItem(object):
    def __init__(self, item, parent=None, index=0, path_label=""):
        ...
        self.index = index
        self.references = []
        self.path_label = path_label  # Store the structural path label

    def get_full_path(self):
        """Get the complete structural path from root to this item"""
        path_segments = []
        current = self
        while current and current.parentItem:
            if current.path_label:
                path_segments.append(current.path_label)
    ...
    def extend(self, items):
        for i in items:
            ...
            # Create appropriate path labels for different item types
            path_label = ""
            if isinstance(i, aaf2.properties.Property):
                path_label = f"{self.get_structural_context()}.{getattr(i, 'name', 'Property')}"
            elif isinstance(i, DummyItem):
                path_label = f"{self.get_structural_context()}.{i.name}"
            elif hasattr(i, 'name') and i.name:
                path_label = f"{self.get_structural_context()}.{i.name}"
            else:
                path_label = f"{self.get_structural_context()}.{self.class_name_of_item(i)}[{index}]"
            
            t = TreeItem(i, self, index, path_label)
            self.children[index] = t
            self.children_count += 1

    def get_structural_context(self):
        """Get the structural context for creating child path labels"""
        if isinstance(self.item, DummyItem):
            return self.item.name
        elif hasattr(self.item, 'name') and self.item.name:
            return self.item.name
        else:
            return self.class_name()

    def class_name_of_item(self, item):
        """Get class name for any item"""
    ...
    def class_name(self):
        return self.class_name_of_item(self.item)
...
class AAFModel(QtCore.QAbstractItemModel):
    def __init__(self, root, parent=None):
        ...
        self.rootItem = TreeItem(root, parent=None, index=0, path_label="Root")
...
class TraceRouteDialog(QtWidgets.QDialog):
    def __init__(self, tree_item, parent=None):
        super().__init__(parent)
        self.setWindowTitle("AAF Trace Route")
        ...
        # Header
        header_label = QtWidgets.QLabel("Structural Path Analysis")
        header_label.setStyleSheet("font-weight: bold; font-size: 14px; padding: 5px;")
        layout.addWidget(header_label)
        
        # Path display
        path_group = QtWidgets.QGroupBox("Hierarchical Path")
        path_layout = QtWidgets.QVBoxLayout(path_group)
        
        self.path_text = QtWidgets.QTextEdit()
        self.path_text.setReadOnly(True)
        self.path_text.setMaximumHeight(100)
        
        # Generate the trace route
        path_info = self._generate_trace_route(tree_item)
        self.path_text.setPlainText(path_info['path'])
        
        path_layout.addWidget(self.path_text)
        layout.addWidget(path_group)
        
        # Detailed breakdown
        details_group = QtWidgets.QGroupBox("Structural Breakdown")
        details_layout = QtWidgets.QVBoxLayout(details_group)
        
        self.details_tree = QtWidgets.QTreeWidget()
        self.details_tree.setHeaderLabels(["Level", "Name", "Class", "Type", "Value"])
        self.details_tree.setAlternatingRowColors(True)
        
        self._populate_details_tree(path_info['breakdown'])
        ...
        copy_path_btn.clicked.connect(lambda: QtWidgets.QApplication.clipboard().setText(path_info['path']))
        
        export_btn = QtWidgets.QPushButton("Export Trace")
        export_btn.clicked.connect(lambda: self._export_trace_route(path_info))
    ...
    def _generate_trace_route(self, tree_item):
        """Generate complete trace route information"""
        path_segments = []
        breakdown = []
        ...
        while current:
            items_in_path.append(current)
            current = current.parentItem
        
        items_in_path.reverse()  # Start from root
        
        for level, item in enumerate(items_in_path):
            item.setup()  # Ensure item is loaded
            
            level_info = {
                'level': level,
                'name': item.name(),
                'class': item.class_name(),
                'type': self._get_item_type(item.item),
                'value': item.properties.get('Value', ''),
                'path_segment': item.path_label if item.path_label else "Root"
            }
            breakdown.append(level_info)
            
            if level > 0:  # Skip root for path display
                path_segments.append(item.path_label if item.path_label else item.name())
        
        path_string = " → ".join(path_segments) if path_segments else "Root"
    ...
    def _get_item_type(self, item):
        """Determine the specific type of AAF item"""
        if isinstance(item, DummyItem):
            return "Reference"
        elif isinstance(item, aaf2.core.AAFObject):
            return "AAF Object"
        elif isinstance(item, aaf2.properties.StrongRefProperty):
    ...
    def _populate_details_tree(self, breakdown):
        """Populate the details tree with breakdown information"""
        for item_info in breakdown:
            tree_item = QtWidgets.QTreeWidgetItem([
                str(item_info['level']),
                item_info['name'],
                item_info['class'],
                item_info['type'],
                str(item_info['value'])[:100] + ("..." if len(str(item_info['value'])) > 100 else "")
            ])
            
            # Set different colors for different levels
            if item_info['level'] == 0:
                tree_item.setBackground(0, QtGui.QColor(240, 248, 255))  # Light blue for root
            elif item_info['level'] % 2 == 0:
                tree_item.setBackground(0, QtGui.QColor(248, 248, 248))  # Light gray for even levels
            
            self.details_tree.addTopLevelItem(tree_item)

    def _export_trace_route(self, path_info):
        """Export trace route to a file"""
        file_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Export Trace Route", "aaf_trace_route.txt", 
            ...
            try:
                if file_path.endswith('.json'):
                    with open(file_path, 'w', encoding='utf-8') as f:
                        json.dump({
                            'path': path_info['path'],
                            'breakdown': path_info['breakdown'],
                            'timestamp': datetime.datetime.now().isoformat()
                        }, f, indent=2)
                else:
                    with open(file_path, 'w', encoding='utf-8') as f:
                        f.write("AAF Structural Trace Route\n")
                        f.write("=" * 50 + "\n\n")
                        ...
                        for item in path_info['breakdown']:
                            f.write(f"Level {item['level']}: {item['name']}\n")
                ...
                QtWidgets.QMessageBox.information(self, "Export Successful", 
                                                 f"Trace route exported to:\n{file_path}")
            except Exception as e:
                QtWidgets.QMessageBox.critical(self, "Export Failed", 
                                             f"Failed to export trace route:\n{str(e)}")
...
class Window(QtWidgets.QTreeView):
    ...
    def showContextMenu(self, point):
        ...
        item_at_point = None
        if index.isValid():
            item_at_point = self.model().getItem(index)
        ...
        # Add trace route action
        traceAction = QtGui.QAction("Trace Route to Here", self)
        traceAction.setShortcut(QtGui.QKeySequence("Ctrl+T"))
        traceAction.triggered.connect(lambda: self.show_trace_route(item_at_point))
        traceAction.setEnabled(bool(item_at_point))
    ...
    @QtCore.Slot()
    def show_trace_route(self, tree_item):
        """Show the trace route dialog for the specified tree item"""
        if not tree_item:
            QtWidgets.QMessageBox.warning(self, "Trace Route", "No item selected for trace route.")
            return
        
        dialog = TraceRouteDialog(tree_item, self)
        dialog.exec()
    ...
    def keyPressEvent(self, event):
        ...
        elif event.key() == QtCore.Qt.Key.Key_T and event.modifiers() == QtCore.Qt.KeyboardModifier.ControlModifier:
            # Ctrl+T for trace route on current selection
            current_index = self.currentIndex()
            if current_index.isValid():
                item = self.model().getItem(current_index)
                self.show_trace_route(item)
        else:
