...
A Tkinter GUI that:
  ...
  - Generates a structurally correct FCPXML with robust still image detection.
"""
...
def create_fcpxml():
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f'        <format height="1080" id="r0" frameDuration="{1}/{int(fps)}s" name="FFVideoFormat1080p25" width="1920"/>\n')
        f.write('        <format height="2160" id="r2" name="FFVideoFormatRateUndefined" width="3840"/>\n')
        ...
        for i, event in enumerate(events_to_process):
            ...
            # --- Robust Still Image Detection ---
            source_name = event.get('Source File Name', '').strip()
            clip_name_check = event.get('Clip Name', '').strip()
            filename_to_check = source_name if source_name else clip_name_check
            ...
            clip_name = sanitize_string(event.get('Clip Name', 'Untitled Asset'))
        ...
        for i, event in enumerate(events_to_process):
            ...
            disk_label = event.get('DiskLabel', '')
            tape_id = event.get('TapeID', '')
            reel_name = ''
            if disk_label and tape_id:
                ...
                reel_name = event.get('Source File Name', '')
            reel_name = sanitize_string(reel_name)

            # --- Robust Still Image Detection (repeated for spine) ---
            source_name = event.get('Source File Name', '').strip()
            clip_name_check = event.get('Clip Name', '').strip()
            filename_to_check = source_name if source_name else clip_name_check
            is_still = filename_to_check.lower().endswith(still_image_extensions)
            ...
            if is_still:
                f.write(f'                        <video start="{clip_start_in_source_frames}/{int(fps)}s" ref="{asset_ref_id}" offset="{clip_offset_frames_relative}/{int(fps)}s" duration="{clip_duration_frames}/{int(fps)}s" name="{clip_name}" enabled="1">\n')
                f.write('                            <adjust-transform position="0 0" scale="1 1" anchor="0 0"/>\n')
                f.write('                        </video>\n')
            else:
                f.write(f'                        <asset-clip start="{clip_start_in_source_frames}/{int(fps)}s" tcFormat="{tc_format}" ref="{asset_ref_id}" offset="{clip_offset_frames_relative}/{int(fps)}s" duration="{clip_duration_frames}/{int(fps)}s" format="r0" name="{clip_name}" enabled="1">\n')
                f.write('                            <adjust-transform position="0 0" scale="1 1" anchor="0 0"/>\n')
                f.write('                            <metadata>\n')
