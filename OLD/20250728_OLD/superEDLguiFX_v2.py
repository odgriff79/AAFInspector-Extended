...
def decode_filepath(filepath_node):
    ...
    Moved to global scope to be accessible by main processing loop.
    """
    try:
        value_node = next(
...
def find_filepath(subnode):
    """
    Deep-finds any Filepath node under a given node.
    Moved to global scope to be accessible by main processing loop.
    """
...
def find_timeline_effects(node, timeline_offset=0, results_list=None):
    ...
    if name == 'Sequence':
        ...
        if attrs and len(attrs) > 3:
            ...
            if '_EFFECT_PLUGIN_NAME' in plugin_keys or '_EFFECT_PLUGIN_CLASS' in plugin_keys: record = True
        if not record:
            op_def = next((c for c in node[3] if c[0] == 'Operation'), None)
            if op_def and len(op_def) > 2 and isinstance(op_def[2], str) and 'MatteKey' in op_def[2]: record = True
        if not record:
            params = next((c for c in node[3] if c[0] == 'Parameters'), None)
            if params and len(params) > 3:
                for p in params[3]:
                    pname = next((x[2] for x in p[3] if x[0]=='Name'), p[0])
                    if 'KEY' in pname.upper(): record = True; break
        if record:
...
class App:
    ...
    def process(self):
        ...
        for effect in all_effects:
            ...
            details['node'] = effect['node'] # Keep the node for later path extraction
        ...
        summary_info = {
            ...
            "Total number of unique sources": len({e["MobID"] for e in events if e.get("MobID") not in ["PanZoomFiller", "PZ_OVERRIDE"]})
        }
        ...
        for idx, e in enumerate(events, 1):
            ...
            is_pan_zoom_override = effect_data and 'Avid Pan & Zoom' in effect_data.get('effect_name', '')
            ...
            # --- NEW LOGIC: Handle Pan & Zoom Override ---
            if is_pan_zoom_override and e.get("MobID") != "PanZoomFiller":
                effect_node = effect_data.get('node')
                file_path = find_filepath(effect_node) or "N/A"
                source_file_name = os.path.basename(file_path) if file_path != "N/A" and file_path else "P&Z Source"
                
                enriched.append({
                    "Event": idx, "Event Name": f"(P&Z Override) {source_file_name}", "Clip Name": source_file_name,
                ...
                # --- Original Logic for Fillers and standard clips ---
                if e.get("MobID") == "PanZoomFiller":
                    ...
                    enriched.append({
                        "Event": idx, "Event Name": "Pan & Zoom on Filler", "Clip Name": source_file_name,
                        "Source File Name": source_file_name, "Source File Path": os.path.dirname(file_path).replace("\\", "/") if file_path != "N/A" and file_path else "N/A",
                        "DiskLabel": "N/A", "TapeID": "N/A", "SourceMobID": "PanZoomFiller", "TrackID": "VFX",
                        "Source Clip EditRate": e["TimelineEditRate"],
                        "Timeline Start TC": frames_to_tc(e["TimelineStartFrame"], timeline_rate, is_drop_frame),
                        "Source Clip start time code": "N/A", "Source Clip offset": "N/A",
                        "StartTime": "N/A", "End Time": "N/A", "Event Length": e["Length"],
        ...
        for r in enriched:
            ...
            if r.get("SourceMobID") not in ["PanZoomFiller", "PZ_OVERRIDE"]:
            ...
            elif r.get("SourceMobID") == "PZ_OVERRIDE":
                 self.log_msg(f"  Source Path (from Effect): {r['Source File Path']}")

            self.log_msg("")

        out_path = os.path.join(output_dir, f"super_edl_fx_report_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
