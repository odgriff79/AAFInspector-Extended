...
A Tkinter GUI that:
  ...
  - Generates a structurally correct FCPXML with fully working keyframe animations,
    correctly handling both timeline-relative and source-relative keyframe data.
"""
...
def parse_keyframe_details(kf_string, clip_start_frames_on_timeline, clip_start_in_source_frames, clip_length_frames):
    ...
    for line in kf_string.strip().split('\n'):
        ...
        if kf_match and current_param:
            ...
            abs_keyframe_frame = int(frame)
            value_str = value_str.strip()
            try:
                value = float(value_str.split('/')[0]) / float(value_str.split('/')[1]) if '/' in value_str else float(value_str)
                
                # --- FINAL FIX for Time Calculation ---
                # This logic now correctly calculates the relative frame within the timeline clip
                relative_frame = abs_keyframe_frame - clip_start_frames_on_timeline
                
                # Ensure the keyframe is within the bounds of the clip on the timeline
                if 0 <= relative_frame <= clip_length_frames:
...
def create_fcpxml():
    ...
    with open(save_path, 'w', encoding='utf-8') as f:
        ...
        for i, event in enumerate(events_to_process):
            ...
            keyframes = parse_keyframe_details(event.get('Keyframe Details', ''), event_start_frames, clip_start_in_source_frames, clip_duration_frames)
