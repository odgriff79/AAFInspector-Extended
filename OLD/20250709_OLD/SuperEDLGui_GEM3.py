...
def extract_metadata(mob_node):
    """Aggressive recursive search for TC Start in any Timecode segment."""
    metadata = {
        "SourceClipStartFrames": 0,
        "URLString": ""
    }

    if not mob_node or not isinstance(mob_node, list):
        print("❌ No mob_node provided.")
        return metadata
    ...
    # --- Recursive timecode Start finder ---
    start_candidates = []

    def walk_for_timecode_starts(n):
        if not isinstance(n, list): return
        if n[0] == "Timecode" and len(n) > 3:
            for child in n[3]:
                if child[0] == "Start":
                    try:
                        val = int(child[2])
                        print(f"🔍 Found Timecode.Start: {val}")
                        start_candidates.append(val)
                    except:
                        print(f"⚠️ Failed to parse Start: {child}")
        children = n[3] if len(n) > 3 else []
        for c in children:
            walk_for_timecode_starts(c)

    walk_for_timecode_starts(mob_node)

    if start_candidates:
        metadata["SourceClipStartFrames"] = max(start_candidates)
    else:
        print("⚠️ No Timecode.Start values found.")
