...
def extract_metadata_with_debug(mob_node):
    """
    Extracts metadata and returns all found start timecodes for debugging.
    """
    ...
    # Find all start values and return the list
    all_starts = []
...
def get_clip_name(mob_map, mobid):
    ...
    # Use a limited version of extract_metadata just for the name to avoid recursion issues
    url = ""
    def find_url(n):
        nonlocal url
        if not isinstance(n, list): return
...
class App:
    ...
    def process(self):
        ...
        self.log_msg("0. Filtering out noisy data (Audio, Data Tracks, Color, etc.)...")
        ...
        if not sequence_mob:
            self.log_msg("❌ Could not find a main sequence mob. The file might not be a valid sequence export, or it may have been filtered out.")
            messagebox.showerror("Error", "Could not find a main sequence mob.")
            return
        ...
        if not events:
            self.log_msg("❌ No clip events were found in the sequence.")
            messagebox.showwarning("Warning", "No clip events were found. The generated report will be empty.")
            return

        enriched = []
        debug_log_lines = [] # <-- For the new debug log
        ...
        for idx, e in enumerate(events, 1):
            ...
            # Append info to the debug log
            debug_log_lines.append(f"Event: {idx} | Clip Name: {clip_name}")
            debug_log_lines.append(f"MobID: {e['MobID']}")
            debug_log_lines.append(f"Found Start Values (frames): {sorted(all_starts)}")
            debug_log_lines.append(f"Selected Value (Max): {source_clip_start_frames}")
            debug_log_lines.append("-" * 50)
            
            enriched.append({
                "Event": idx,
                "SourceFile": clip_name,
        ...
        if not enriched:
            self.log_msg("❌ No data to write to CSV.")
            messagebox.showinfo("Done", "Processing complete, but no events were found to create a report.")
            return
            
        # --- Save the debug log file ---
        out_debug = os.path.join(os.path.dirname(self.json_path), f"timecode_debug_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        self.log_msg(f"4. Saving TC debug log to {os.path.basename(out_debug)}...");
        try:
            with open(out_debug, "w", encoding="utf-8") as f:
                f.write("\n".join(debug_log_lines))
        except Exception as e:
            self.log_msg(f"❌ Error saving debug log file: {e}")

        # --- Save the main CSV report ---
        out_csv = os.path.join(os.path.dirname(self.json_path), f"super_edl_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv")
        self.log_msg(f"5. Saving report to {os.path.basename(out_csv)}...");
        try:
            with open(out_csv, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(f, fieldnames=list(enriched[0].keys())); writer.writeheader(); writer.writerows(enriched)
        except Exception as e:
            self.log_msg(f"❌ Error saving CSV file: {e}")
            messagebox.showerror("Error", f"Could not save the CSV report:\n{e}")
            return
