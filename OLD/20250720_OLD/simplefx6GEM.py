...
def analyze_effect_parameters(op_group_node):
    ...
    if params_node and len(params_node) > 3:
        plugin_name_node = next(
            (p for p in params_node[3] if isinstance(p, list) and p[0] in ('_EFFECT_PLUGIN_NAME', '_EFFECT_NAME')),
            None
        )
        if plugin_name_node and len(plugin_name_node) > 2:
            effect_name = plugin_name_node[2]
    ...
    if animated_params or effect_name != "Unknown Effect":
            ...
            if not event['effects']:
                ...
                for effect in event['effects']:
                    ...
                    for param_name, keyframes in effect['animated_params'].items():
                        ...
                        for kf in keyframes:
                            self.log_msg(f"       > Time: {kf['time']}, Value: {kf['value']}")
            self.log_msg("")
