...
def extract_keyframes_from_parameter(param):
    keyframes = []
...
def analyze_effect_parameters(op_group_node):
    ...
    if params_node and len(params_node) > 3:
        for param in params_node[3]:
            if not (isinstance(param, list) and isinstance(param[0], str)):
                continue
            param_name = param[0]

            if effect_name == '3DWarp' and param_name not in FILTER_PARAMS:
                continue

            keyframes = extract_keyframes_from_parameter(param)
            if keyframes:
                animated_params[param_name] = keyframes
                print(f"[DEBUG] Found parameter: {param_name} with {len(keyframes)} keyframes")

    if effect_name == 'Resize':
        static_params = attributes_info

    return {"effect_name": effect_name, "animated_params": animated_params, "static_params": static_params}
    ...
    def load_and_process(self):
        ...
        for idx, event in enumerate(events, 1):
            ...
            for effect in valid_effects:
                ...
                if effect['static_params']:
                    for attr, val in effect['static_params'].items():
                        output_lines.append(f"Static Parameter: {attr} = {val}")
