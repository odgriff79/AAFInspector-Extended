...
# -*- coding: utf-8 -*-
"""
csv_to_fcpxml_and_markers_REL13_REL15.py

CSV → FCPXML (REL13) + Resolve Marker EDL (REL15)

What's new vs prior builds:
- Robust AFX param parsing (POS/ SCALE / ROT / OPACITY / CROP) from "Keyframe Details"
  including exact Avid-style tokens like AFX_POS_X_U, AFX_SCALE_Y_U, AFX_CROP_LEFT_U, etc.
- Unicode-safe normalization of "Keyframe Details" (NBSP, en/em dashes, bullets, etc.)
- Guaranteed animated output when animation exists (fills tracks and writes <param><keyframeAnimation>)
- time_mode switch: "abs" (Resolve style, default) or "local" (clip-relative)
- pos_units: "pixels" (Resolve center-pixel, default) or "percentH" (percent-of-height micro units)
- Crop export (<adjust-crop><trim-rect> with animated left/right/top/bottom)
- Flip/Flop/Flip-Flop: static mirrors merged by sign-multiplying scale (on clip or overlapping filler)
"""
...
def norm_ws(s: str) -> str:
    ...
    # Normalize unicode spaces and dashes/bullets that often appear in Keyframe Details
    s = s.replace("\u00A0", " ")   # NBSP
...
def frames_to_tc_df(frames: int, fps: float) -> str:
    # 29.97/59.94 support; otherwise fall back to NDF-like formatting
...
def frames_to_tc(frames: int, fps: float, drop_frame: bool = False, df: Optional[bool] = None) -> str:
    """Accepts df kwarg (compat with GUI)."""
    if df is not None:
        drop_frame = df
...
# Direct AFX tokens we will recognize explicitly
AFX_MAP = {
    # Position
    "AFX_POS_X_U": ("pos", "x"),
    "AFX_POS_Y_U": ("pos", "y"),
...
# Fuzzy matches (lowercased contains) to cover non-AFX names we’ve seen
FUZZY = {
    "posx": ("pos", "x"), "pos_x": ("pos", "x"), "position x": ("pos","x"), "center x": ("pos","x"),
    "posy": ("pos", "y"), "pos_y": ("pos", "y"), "position y": ("pos","y"), "center y": ("pos","y"),
    "position": ("pos", None), "center": ("pos", None),

    "scale x": ("scale","x"), "scale y": ("scale","y"), "zoom x": ("scale","x"), "zoom y": ("scale","y"),
    "scale": ("scale", None), "zoom": ("scale", None), "resize": ("scale", None), "size": ("scale", None),

    "rotation": ("rot", None), "rotate": ("rot", None), "angle": ("rot", None),

    "opacity": ("opacity", None), "amount": ("opacity", None), "blend": ("opacity", None),
...
def classify_param_name(pname: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    for needle, cls in FUZZY.items():
        if needle in low:
            return cls
    # Fallback: try to detect suffixes like "..._X" or "..._Y"
...
def convert_for_fcpxml(param_group: str, raw_value: Any) -> Optional[float]:
    v = to_float_safe(raw_value)
    if v is None:
        return None
    if param_group == "opacity":
        return v / 100.0 if v > 1.0 else v
    if param_group == "scale":
...
def avid_pos_to_resolve_pixels(px_raw: Any, py_raw: Any, width: int, height: int) -> Tuple[Optional[float], Optional[float]]:
...
def parse_flip_flags(effect_name: str) -> Tuple[bool, bool]:
    ...
    if "flip flop" in n or "flip-flop" in n:
        return True, True
...
def parse_keyframes_block(kf_text: str) -> Dict[str, List[Tuple[int, str]]]:
    """Return dict: param_name -> [(abs_frame, value_str), ...]"""
    ...
    for raw in txt.splitlines():
        ...
        if line.startswith("--- Static Parameters ---"):
            in_anim = False; cur = None; continue
        ...
        if m_param:
            ...
            continue
        if in_anim and cur:
            m_kf = KF_LINE_RE.search(line)
            if m_kf:
                fr = int(m_kf.group(1))
                val = m_kf.group(2).strip()
                animated[cur].append((fr, val))
    # sort each list by absolute frame
    for k in animated:
...
def group_raw_kf_by_time(animated: Dict[str, List[Tuple[int, str]]]) -> "OrderedDict[int, Dict[str, Any]]":
    """Return OrderedDict[abs_frame] -> slots dict (posx,posy,scalex,scaley,rot,op,cropleft,cropright,croptop,cropbottom)"""
        ...
        for t, val in pts:
            ...
            if cls == "pos":
                if axis == "x": g['posx'] = val
                    ...
                    g['posx'] = val if g['posx'] is None else g['posx']
                    g['posy'] = val if g['posy'] is None else g['posy']
            elif cls == "scale":
...
def build_model_from_csv(csv_path: Path, timeline_width: int = 1920, timeline_height: int = 1080,
                         pos_units: str = "pixels") -> Tuple[Dict[str, Any], List[EventModel]]:
    ...
    # collect flip/flop on filler (adjustment ranges)
    ...
    for r in events_csv:
        ...
        if is_filler(r):
            if h or v:
                ...
                try:
                    dur = int(str(r.get("Event Length", "0")).split(".")[0] or "0")
                except:
                    dur = 0
    ...
    for r in rows:
        ...
        for absf, comp in M.kf_raw_grouped.items():
            ...
            if local < 0:
                local = 0
            if local >= M.evt_len:
                local = max(0, M.evt_len - 1)

            # position
            px, py = avid_pos_to_resolve_pixels(comp['posx'], comp['posy'], timeline_width, timeline_height)
            # Convert to percent-of-height if requested
            if pos_units == "percentH":
                px, py = pixels_to_percentH(px, py, timeline_height)
            if px is None:
                px = last_px
            if py is None:
                py = last_py
            ...
            # if a single scale recorded, apply both axes
            ...
            def crop_val(v):
                ...
                if f is None:
                    return None
                if abs(f - int(round(f))) < 1e-6:
                    return str(int(round(f)))
                return fmt_float(f, 3)

            for side, key in (("left","cropleft"), ("right","cropright"),
                              ("top","croptop"), ("bottom","cropbottom")):
                raw = comp[key]
                if raw is not None:
                    cv = crop_val(raw)
                    if cv is not None:
                        if side == "left":   M.tr_crop_left.append((local, cv))
                        if side == "right":  M.tr_crop_right.append((local, cv))
                        if side == "top":    M.tr_crop_top.append((local, cv))
                        if side == "bottom": M.tr_crop_bottom.append((local, cv))

        # clip-level and overlapping filler flips
        h_clip, v_clip = parse_flip_flags(M.effect_name)
        h_sign, v_sign = (-1 if h_clip else 1), (-1 if v_clip else 1)
        # Gather overlapping filler-based flips
        if filler_flip_segments:
            m_start = M.evt_abs
            m_end = M.evt_abs + max(0, M.evt_len)
            for fs, fe, fh, fv in filler_flip_segments:
                if not (fe <= m_start or fs >= m_end):
                    if fh: h_sign *= -1
                    if fv: v_sign *= -1
        M.scale_sign_x = h_sign
        M.scale_sign_y = v_sign

        # apply signs to scale keyframes
        if M.tr_scale:
            new_scale = []
            for t, val in M.tr_scale:
                try:
                    sx_s, sy_s = val.strip().split()
                    sx = float(sx_s); sy = float(sy_s)
                except Exception:
                    sx, sy = 1.0, 1.0
                sx *= M.scale_sign_x
                sy *= M.scale_sign_y
                new_scale.append((t, fmt_pair(sx, sy, 2)))
            M.tr_scale = new_scale
...
def write_fcpxml_from_model(header: Dict[str, Any], models: List[EventModel], out_path: Path,
                            time_mode: str = "abs", export_crop: bool = True) -> None:
    ...
    def write_kf_param(M: EventModel, name: str, track: List[Tuple[int, str]]):
        if not track:
            return
        # write base attribute value at first element
        L.append(f'                                <param name="{name}" value="{track[0][1]}">')
        L.append('                                    <keyframeAnimation>')
        for local, val in track:
            t = (M.evt_abs + local) if time_mode == "abs" else local
            # clamp inside [in, out-1]
            if t >= M.evt_abs + M.evt_len:
                t = M.evt_abs + max(0, M.evt_len - 1)
            if t < M.evt_abs:
                t = M.evt_abs
    ...
    for i, M in enumerate(models):
        ...
        # crop (if any) — write animated sides present
        if export_crop and (M.tr_crop_left or M.tr_crop_right or M.tr_crop_top or M.tr_crop_bottom):
            ...
            def write_crop_param(side_name: str, track: List[Tuple[int, str]]):
                if not track:
                    return
                L.append(f'                                    <param name="{side_name}" value="{track[0][1]}">')
                L.append('                                        <keyframeAnimation>')
                for local, val in track:
                    t = (M.evt_abs + local) if time_mode == "abs" else local
                    if t >= M.evt_abs + M.evt_len:
            ...
            write_crop_param("left",   M.tr_crop_left)
            write_crop_param("right",  M.tr_crop_right)
            write_crop_param("top",    M.tr_crop_top)
            write_crop_param("bottom", M.tr_crop_bottom)

            L.append('                                </trim-rect>')
            L.append('                            </adjust-crop>')

        # transform
        L.append(f'                            <adjust-transform scale="{base_scale_attr}" position="{base_pos}" anchor="0 0">')
        ...
        if M.tr_opacity:
            ...
            L.append(f'                                <param name="amount" value="{base_op}">')
            L.append('                                    <keyframeAnimation>')
...
def write_marker_edl_from_model(header: Dict[str, Any], models: List[EventModel],
    ...
    for idx, M in enumerate(models, start=1):
        ...
        if mirrors:
            L.append(f"# MIRROR: {'&'.join(mirrors)}")

        # count all Avid-side slots present at each grouped time
        kf_count = 0
        for comp in M.kf_raw_grouped.values():
            kf_count += int(comp['posx'] is not None) + int(comp['posy'] is not None)
            kf_count += int(comp['scalex'] is not None) + int(comp['scaley'] is not None)
            kf_count += int(comp['rot'] is not None) + int(comp['op'] is not None)
            kf_count += int(comp['cropleft'] is not None) + int(comp['cropright'] is not None)
            kf_count += int(comp['croptop'] is not None) + int(comp['cropbottom'] is not None)
        ...
        # Resolve-space lookups at absolute frames for display
        ...
        op_at  = {M.evt_abs + t: v for t, v in M.tr_opacity}
        cl_at  = {M.evt_abs + t: v for t, v in M.tr_crop_left}
        cr_at  = {M.evt_abs + t: v for t, v in M.tr_crop_right}
        ct_at  = {M.evt_abs + t: v for t, v in M.tr_crop_top}
        cb_at  = {M.evt_abs + t: v for t, v in M.tr_crop_bottom}

        for kf_idx, (absf, raw) in enumerate(times, start=1):
            if absf >= M.evt_abs + M.evt_len:
                absf = M.evt_abs + max(0, M.evt_len - 1)
            avid_tc = frames_to_tc(absf, fps, df=df)
            rel_tc = frames_to_tc(max(0, min(M.evt_len - 1, absf - M.evt_abs)), fps, df=df)
            ...
            if raw['rot'] is not None: avid_parts.append(f"rotation={raw['rot']}")
            if raw['op'] is not None: avid_parts.append(f"opacity={raw['op']}")
            if raw['cropleft'] is not None or raw['cropright'] is not None or raw['croptop'] is not None or raw['cropbottom'] is not None:
                avid_parts.append("crop={L:%s R:%s T:%s B:%s}" % (
                    raw.get('cropleft', '-'), raw.get('cropright', '-'),
                    raw.get('croptop', '-'),  raw.get('cropbottom', '-')
                ))
            if not avid_parts:
                avid_parts.append("-")

            res_parts = []
            if absf in pos_at: res_parts.append(f'position="{pos_at[absf]}"')
            if absf in scl_at: res_parts.append(f'scale="{scl_at[absf]}"')
            if absf in rot_at: res_parts.append(f'rotation="{rot_at[absf]}"')
            if absf in op_at:  res_parts.append(f'opacity="{op_at[absf]}"')
            if absf in cl_at:  res_parts.append(f'cropL="{cl_at[absf]}"')
            if absf in cr_at:  res_parts.append(f'cropR="{cr_at[absf]}"')
            if absf in ct_at:  res_parts.append(f'cropT="{ct_at[absf]}"')
            if absf in cb_at:  res_parts.append(f'cropB="{cb_at[absf]}"')
...
def main() -> None:
    ...
    ap.add_argument("-i", "--input", required=True, type=Path, help="Input CSV (SuperEDL CSV)")
    ap.add_argument("-x", "--xml", required=True, type=Path, help="Output FCPXML (REL13)")
    ap.add_argument("-e", "--edl", required=True, type=Path, help="Output marker EDL (REL15)")
    ap.add_argument("--width", type=int, default=1920)
    ap.add_argument("--height", type=int, default=1080)
    ap.add_argument("--max-kf", type=int, default=None)
    ap.add_argument("--time-mode", choices=["abs", "local"], default="abs",
                    help='Keyframe time domain: "abs" (sequence/Resolve) or "local" (clip-relative)')
    ap.add_argument("--pos-units", choices=["pixels", "percentH"], default="pixels",
                    help='Position units: "pixels" (Resolve) or "percentH" (percent-of-height micro units)')
    ap.add_argument("--no-crop", action="store_true", help="Disable writing <adjust-crop> even if crop KFs exist")
    args = ap.parse_args()
    ...
    print(f"Options: time_mode={args.time_mode}, pos_units={args.pos_units}, crop_export={not args.no_crop}")
