...
def create_xmeml_final(df, logger):
    ...
    # Filter out fillers, slugs, AND placeholders
    original_row_count = len(df)
    filler_conditions = (df[C['clip_name']].str.lower() == 'filler') | (df[C['clip_name']].str.lower() == 'slug')
    placeholder_conditions = (
        df[C['clip_name']].str.contains('placeholder', case=False, na=False) |
        (df[C['source_file_path']].str.contains('placeholders', case=False, na=False) if C['source_file_path'] in df.columns else False)
    )
    df_real_clips = df[~(filler_conditions | placeholder_conditions)].copy()

    # Remove duplicates, keeping the last instance
    df_real_clips = df_real_clips.sort_values(by=C['start_time_frames'])
    df_real_clips = df_real_clips.drop_duplicates(subset=[C['clip_name']], keep='last')

    omitted_count = len(df) - len(df_real_clips)
    if omitted_count > 0:
        logger(f"INFO: Kept {len(df_real_clips)} real source clips and omitted {omitted_count} other rows.")

    if df_real_clips.empty:
        ET.SubElement(sequence, 'name').text = "Empty Sequence (No real source clips found)"
    ...
    for col_name in numeric_cols:
        df_real_clips[col_name] = pd.to_numeric(df_real_clips[col_name], errors='coerce').fillna(0)

    # Correctly determine the first clip and its timecode
    first_clip_row = df_real_clips.loc[df_real_clips[C['start_time_frames']].idxmin()]
    ...
    last_clip = df_real_clips.loc[df_real_clips[C['start_time_frames']].idxmax()]
    ...
    ET.SubElement(sequence, 'rate').text = str(int(sequence_rate_val))
    ...
    for i, row in df_real_clips.iterrows():
